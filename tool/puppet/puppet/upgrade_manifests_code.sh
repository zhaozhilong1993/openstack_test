...
## upgeade karam
cd /opt/puppet/pre_production/karam
modify_karam=`git status |grep 'modified'`
if [[ $modify_karam != 0 ]]; then
  fatal "has modify in karam local-----"
else
  /etc/init.d/httpd stop
  git pull
  sleep 5
  ...
  git submodule init
  git submodule update
  ln -sd /opt/puppet/pre_production/karam/* /etc/puppet/environments/pre_production/modules/
  rm -f /etc/puppet/environments/pre_production/modules/README.md /etc/puppet/environments/pre_production/modules/Release.md
  /etc/init.d/httpd start
fi
